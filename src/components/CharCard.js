...
export default function CharCard(props) {
  ...
  const boots = items.filter(item => item.type === 'boots' && item.is_weared)[0]
  const gloves = items.filter(item => item.type === 'gloves' && item.is_weared)[0]
  ...
  return (
    <StyledField>
      ...
      <div className="character">
        ...
        <div className="char-middle">
          <div className="char-left">
            ...
            <div data-tip="Your Exp" data-type="info" className="exp">
              <div style={{ height: `${current_exp / exp * 150}px` }}></div>
        ...
        {
          freeStats ? 
          <span className='free-stats'>{freeStats} points</span> : null
        }
        ...
        <div className="char-bottom">
          ...
          <div className="stats">
            <div data-tip="Strength affects the chance of crit and damage to you" data-type="info">strength: {str}{freeStats ? <button onClick={() => updateStats('str')} className="add_stats">+</button> : null}</div>
            <div data-tip="Vitality affects the power of crit and your HP" data-type="info">vitality: {vit}{freeStats ? <button onClick={() => updateStats('vit')} className="add_stats">+</button> : null}</div>
            <div data-tip="Dextirity affects the power of crit and the chance to dodge a blow" data-type="info">dextirity: {dex}{freeStats ? <button onClick={() => updateStats('dex')} className="add_stats">+</button> : null}</div>
            <div data-tip="Accuracy affects the chance of crit and the chance to hit the enemy" data-type="info">accuracy: {acc}{freeStats ? <button onClick={() => updateStats('acc')} className="add_stats">+</button> : null}</div>
            <div data-tip="Your Dmg" data-type="info">weapon dmg: {dmg}</div>
          </div>
        </div>
