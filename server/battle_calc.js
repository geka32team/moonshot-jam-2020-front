...
function calc (lvl, dmg, dex, str, acc, vit, e_lvl, e_str, e_acc, answer_time = 5, interval = 6, is_char = false) {
    if(!is_hit(dex, e_acc) || (answer_time > interval && is_char)) return [0, false]

    let is_crit = crit_chance(str, acc, lvl) > Math.random()

    let damage = dmg_char(dmg, e_str, e_lvl) * (is_crit ? crit_power(vit, dex, lvl) : 1)
